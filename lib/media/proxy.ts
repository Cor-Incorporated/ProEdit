 ...
 */
export async function generateProxyForMedia(mediaFileId: string): Promise<void> {
  const supabase = await createClient();
  ...
  // Avoid regenerating if another process already started or proxy exists
  if (media.proxy_status === "processing" || (media.proxy_status === "ready" && media.proxy_path)) {
    return;
  }
  ...
  const proxyTempPath = `${sourcePath}-proxy.mp4`;

  try {
    await transcodeProxy(sourcePath, proxyTempPath, 1280, 720);
