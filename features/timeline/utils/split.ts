...
export function splitEffect(
  ...
  // FIX: Round all integer values to prevent database errors
  // Database schema requires INTEGER types for these fields
  const roundedRelativePosition = Math.round(relativePosition)
  const roundedRemainingDuration = Math.round(effect.duration - relativePosition)

  // Left effect (keeps original ID)
  const leftEffect: Effect = {
    ...effect,
    duration: roundedRelativePosition,
    end: Math.round(effect.start + relativePosition),
  }
  ...
  const rightEffect: Effect = {
    ...
    start_at_position: Math.round(splitTimecode),
    duration: roundedRemainingDuration,
    start: Math.round(effect.start + relativePosition),
