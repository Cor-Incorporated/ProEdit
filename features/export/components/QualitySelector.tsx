...
export function QualitySelector({ value, onValueChange, disabled = false }: QualitySelectorProps) {
  return (
    <div className="space-y-3">
      <Label className="text-sm font-medium">エクスポート品質</Label>
      <RadioGroup
        value={value}
      ...
      >
        {Object.entries(EXPORT_PRESETS).map(([key, preset]) => (
          <div key={key} className="flex items-center space-x-3 space-y-0">
            ...
            <Label htmlFor={key} className="font-normal cursor-pointer flex flex-col">
